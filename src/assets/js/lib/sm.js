...
//          debugger;
            params['closed']&&params['closed'].call();
        })
        $.openModal(modal);
        return modal[0];
    };
...
//      debugger;
    ...
    function handleClicks(e) {
        ...
        if (clicked.hasClass('modal-overlay')) {
            ...
            if ($('.modal-overlay-visible').length > 0 && defaults.modalCloseByOutside)
...
+ function($) {
    ...
    var Picker = function (params) {
        ...
        p.updateValue = function () {
            ...
            if (p.input && p.input.length > 0) {
                if(p.cols.length==1){
                    newRealValue=p.cols[0].realValue;
                    if(newRealValue=="01") {
                        $(".showGender").hide();
                    }else{
                        $(".showGender").show();
                    }
                    $(p.input).attr("data-option",newRealValue);
                }
        ...
        p.initPickerCol = function (colElement, updateItems) {
            ...
            col.updateItems = function (activeIndex, translate, transition, valueCallbacks) {
                ...
                if (valueCallbacks || typeof valueCallbacks === 'undefined') {
                    ...
                    col.realValue = selectedItem.attr('data-picker-option');
        ...
        p.columnHTML = function (col, onlyItems) {
            ...
            if (col.divider) {
                ...
                for (var j = 0; j < col.values.length; j++) {
                    var colvalues = "";
                    if(typeof col.values[j] =="string" && col.values[j].indexOf('-')>-1){
                        colvalues= col.values[j].split("-");
                        columnItemsHTML += '<div class="picker-item" data-picker-option="'+colvalues[0]+'"data-picker-value="' + colvalues[1] + '">' + (col.displayValues ? col.displayValues[j] : colvalues[1]) + '</div>';
                    }else{
...
+ function($) {
    ...
    $.fn.datetimePicker = function(params) {
        return this.each(function() {
            ...
            $(this).picker(p);
            if (params&&params.value) $(this).val(p.formatValue(p, p.value, p.value));
        });
    };
